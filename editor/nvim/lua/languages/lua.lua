local default_workspace = {
    library = {
        vim.fn.expand "$VIMRUNTIME", require("neodev.config").types(),
        "${3rd}/busted/library", "${3rd}/luassert/library", "${3rd}/luv/library"
    },
...
local add_packages_to_workspace = function(packages, config)
    ...
    local runtimedirs = vim.api
                            .nvim__get_runtime({"lua"}, true, {is_lua = true})
...
local opts = {
    settings = {
        Lua = {
            ...
            diagnostics = {globals = {"vim", "lvim", "reload"}},
